...
def build_client(provider: str):
    """
    根据 provider 名称构建 LLM 客户端（与 run_function_call.py 完全一致）。
...
def build_server_configs() -> dict[str, StdioServerParameters]:
    """
    构建所有 MCP Server 的启动配置。
    
    Returns:
        {server_label: StdioServerParameters} 字典
        
    说明：
      - 每个 Server 都是一个 Python 脚本，由 Host 作为子进程启动
      - stdio 通信：Host 通过子进程的 stdin/stdout 发送 JSON-RPC 消息
      - env 继承当前环境变量（确保 DASHSCOPE_API_KEY 等能传到子进程）
    """
...
async def connect_all_servers(stack: AsyncExitStack):
    """
    连接所有 MCP Server，完成工具发现与 schema 转换。
    
    流程（教学时刻 2）：
      1. stdio_client 建立进程间通信管道（子进程的 stdin/stdout）
      2. ClientSession 初始化会话
      3. initialize() 握手，协商协议版本和能力
      4. list_tools() 发现工具，获取每个工具的 inputSchema
      5. 将 MCP inputSchema 适配成 OpenAI tools schema（直接塞，因为都是 JSON Schema）
    
    Args:
        stack: AsyncExitStack，用于统一管理多个 Server 子进程的生命周期
    
    Returns:
        (tool_registry, openai_tools):
          tool_registry: {tool_name: (ClientSession, server_label)}，用于路由 call_tool
          openai_tools: 转换后的 OpenAI tools schema 列表，直接喂给 LLM
    """
...
SYSTEM_PROMPT = (
    ...
    "涉及位置/天气时：只要位置信息用 get_city_position；要天气+位置用 get_city_weather。"
...
async def run(client, model: str, question: str,
    ...
    与 run_function_call.py 的差异（教学时刻 3）：
      - 工具来源：MCP 协议发现，非手写 schema
      - 工具执行：通过 call_tool 跨进程调用 Server，非直接调后端函数
    
    Args:
        client: OpenAI 客户端实例
        model: 模型名称字符串
        question: 用户问题
        tool_registry: 工具路由表 {tool_name: (session, label)}
        openai_tools: OpenAI tools schema 列表
        verbose: 是否打印中间过程
...
async def main_async(provider: str, question: str | None, demo: bool, verbose: bool, as_json: bool):
    """
    异步主函数：连接所有 Server，执行问题并输出结果。
    
    Args:
        provider: LLM 提供商
        question: 单个问题（或 None）
        demo: 是否跑内置示例问题集
        verbose: 是否打印中间过程
        as_json: 是否输出 JSON（供 compare.py 解析）
    """
