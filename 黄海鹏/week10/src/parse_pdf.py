...
class ParsedBlock:
    """
    一个解析块 = 报告里的一段连续内容（文字段落 or 表格）

    保留 page_num 和 section_path 非常重要——
    RAG 答案引用时能告诉用户"来自第38页，市场分析/用户规模"
    """
...
NOISE_PATTERNS = [
    re.compile(r"^.{1,40}(年度报告|白皮书|报告)\s*$"),    # 页眉：报告名+年度报告/白皮书
...
def main():
    ...
    if manifest_path.exists():
        ...
        # 没有 manifest 就直接扫目录
        manifest = [
            {"filename": p.name, "source": p.stem, "year": ""}
            for p in RAW_DIR.glob("*.pdf")
        ]

    if not manifest:
        logger.error("没有找到任何 PDF，请确认 data/raw_pdf/ 目录下有 PDF 文件")
        return
