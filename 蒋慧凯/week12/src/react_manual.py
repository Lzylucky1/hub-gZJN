...
- 多轮对话时，注意上下文中的历史问题和答案，合理理解指代（如"那它呢""比较两者"）
"""
...
def run(
    ...
    for step in range(1, max_steps + 1):
        ...
        memory.add_assistant_message(llm_output)
        ...
        if parsed["type"] == "unparseable":
            ...
            memory.add_tool_observation(observation)
            yield {
                "step":        step,
                "type":        "error",
        ...
        memory.add_tool_observation(str(observation))
        ...
        # 更新下一轮要用的上下文（已存入 memory，下一循环会重新拉取）
        messages = memory.get_context_messages(SYSTEM_PROMPT)
