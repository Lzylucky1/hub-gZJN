...
# 默认与手写版保持一致：使用 DashScope + qwen-max
client = OpenAI(
...
# 如需使用 DeepSeek，取消下面注释并注释掉上方 DashScope 配置
# client = OpenAI(
...
def run(
    ...
    参数：
      memory: 可选 ConversationMemory 实例，用于多轮对话上下文。
    ...
    messages = memory.get_context_messages(FC_SYSTEM_PROMPT)
    final_answer = None
    ...
    for step in range(1, max_steps + 1):
        ...
        if reason == "stop" or not msg.tool_calls:
            ...
            memory.add_turn_summary(question, final_answer)
            yield {
                "step":   step,
                "type":   "final",
                "thought": "",
        ...
        # 模型请求调用工具：保存 assistant 消息（含 tool_calls）
        tool_calls_payload = [
            {
                "id": tc.id,
                "type": "function",
        ...
        memory.add_assistant_tool_calls(msg.content or "", tool_calls_payload)
        ...
        for tool_call in msg.tool_calls:
            ...
            memory.add_tool_message(str(observation), tool_call.id)
    ...
    memory.add_turn_summary(question, f"已达最大步数 {max_steps}，未能得出最终答案")
    yield {
        "step":   max_steps + 1,
        "type":   "max_steps",
