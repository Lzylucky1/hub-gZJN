...
# ── 路径配置 ──────────────────────────────────────────────────────────────────
HOMEWORK_BASE_DIR = Path(__file__).parent.parent

# 优先使用 homework 目录下的 vectorstore；若不存在，回退到原项目目录
# （避免大文件重复拷贝，原项目位置固定为 "week12 agent/react_financial_agent"）
_ORIGIN_PROJECT_DIR = HOMEWORK_BASE_DIR.parent / "week12 agent" / "react_financial_agent"

if (HOMEWORK_BASE_DIR / "vectorstore" / "faiss_index.bin").exists():
    VECTORSTORE_DIR = HOMEWORK_BASE_DIR / "vectorstore"
else:
    VECTORSTORE_DIR = _ORIGIN_PROJECT_DIR / "vectorstore"


def _prepare_faiss_dir(src_dir: Path) -> Path:
    """
    FAISS 在 Windows 下对中文/空格路径支持不稳定，
    若源路径包含非 ASCII 或空格，则复制到临时 ASCII 目录后加载。
    """
    src_str = str(src_dir)
    # 判断是否需要复制：路径含非 ASCII 字符或空格
    if all(ord(c) < 128 for c in src_str) and " " not in src_str:
        return src_dir

    tmp_root = Path(tempfile.gettempdir())
    tmp_dir = tmp_root / f"react_faiss_{src_dir.stat().st_mtime}"
    tmp_dir.mkdir(parents=True, exist_ok=True)

    for fname in ("faiss_index.bin", "faiss_meta.json"):
        src = src_dir / fname
        dst = tmp_dir / fname
        if not dst.exists() or dst.stat().st_mtime < src.stat().st_mtime:
            shutil.copy2(str(src), str(dst))

    logger.info(f"FAISS 索引已复制到 ASCII 临时目录: {tmp_dir}")
    return tmp_dir


VECTORSTORE_DIR = _prepare_faiss_dir(VECTORSTORE_DIR)
logger.info(f"使用向量索引目录: {VECTORSTORE_DIR}")
