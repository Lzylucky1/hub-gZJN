...
def get_weather(city: str | list[str] | tuple[str, ...] | set[str] | Iterable[str]) -> str:
    """
    查询一个或多个城市的当前天气及未来3天预报。

    Args:
        city: 城市名称，支持中文，例如 "宁德"、"北京"、"上海"；也可以传入多个城市。

    Returns:
        包含温度、湿度、风速、天气状况和3天预报的文字描述；多个城市会依次拼接输出。
    """
    ...
    if len(cities) == 1:
        return _get_weather_single(cities[0])

    reports = []
    for idx, item in enumerate(cities, 1):
        reports.append(f"===== 第 {idx} 个城市：{item} =====\n{_get_weather_single(item)}")
    return "\n\n".join(reports)


def _get_weather_single(city: str) -> str:
