...
TOOLS_SCHEMA = [
    ...
    },
    {
        ...
        "function": {
            "name": "get_weather",
            "description": "查询指定城市的当前天气及未来3天预报。城市用中文名，如 '宁德'、'北京'。如果需要多次查询天气，可以连续调用本工具。",
            "parameters": {
                "type": "object",
                "properties": {
...
SYSTEM_PROMPT = (
    ...
    "涉及天气时调用 get_weather；如果需要查询多个城市或继续补充信息，可以多轮循环调用 get_weather，"
    "直到工具不再返回新的调用需求为止。"
)
...
def run(client, model: str, question: str, verbose: bool = True) -> dict:
    """
    多轮闭环：提问 → 模型输出 tool_call → 执行 → 回填 → 继续让模型决定是否再调用工具 → 最终回答。
    ...
    answer = (msg.content or "") if msg is not None else ""
