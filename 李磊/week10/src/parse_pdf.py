"""
PDF 解析脚本：将原始 PDF 转换为结构化文本

教学重点（企业级 RAG 的真实挑战）：
  1. 数字 PDF vs 扫描件：处理方式完全不同
  2. 表格提取：捕获文档中的表格，避免文字流乱序
...
def _extract_day(filename: str) -> str:
    """从 MML_DAY1.pdf 提取 DAY1"""
    m = re.search(r'DAY(\d+)', filename, re.IGNORECASE)
    return f"DAY{m.group(1)}" if m else "unknown"
...
def main():
    ...
    if manifest_path.exists():
        ...
        manifest = [
            {"filename": p.name, "day": _extract_day(p.name)}
            for p in RAW_DIR.glob("*.pdf")
        ]

    if not manifest:
        logger.error("没有找到任何 PDF")
        return
