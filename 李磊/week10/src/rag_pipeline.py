...
使用方式：
  ...
  python rag_pipeline.py --query "ADAM优化器是什么"
  python rag_pipeline.py --query "..." --day DAY3
...
EMBED_DIM       = 1024
LLM_MODEL       = "qwen-max"         # qwen-turbo（更快）/ qwen-plus（均衡）/ qwen-max（更强）

TOP_K_RETRIEVE  = 10    # 初始召回数
TOP_K_RERANK    = 4     # Rerank 后保留数（给 LLM）
SCORE_THRESHOLD = 0.25  # 最高得分低于此值时触发拒绝回答（归一化余弦相似度）

SYSTEM_PROMPT = """你是一个专业的机器学习/深度学习教学助手，专门回答培训课件中的问题。

回答规则：
1. 只根据【参考资料】中的内容回答，不得引用或编造课件外的知识
2. 若参考资料不足以支撑回答，直接说"根据提供的课件资料无法回答此问题"
3. 引用具体内容时，在句末标注来源编号，如：ADAM优化器通过动量和二阶矩来加速收敛[1]
4. 回答简洁清晰，重点突出，避免无关废话
5. 涉及公式或算法时，用通俗语言辅助解释"""


# ── DashScope 客户端 ──────────────────────────────────────────────────────────

_DEFAULT_API_KEY = "xxx"

def get_client() -> OpenAI:
    api_key = os.getenv("DASHSCOPE_API_KEY") or _DEFAULT_API_KEY
...
class RAGPipeline:
    ...
    def query(
        ...
        if top_score < SCORE_THRESHOLD and filter_meta is None:
            return {
                "answer": "根据课件知识库未能找到与该问题相关的内容，建议直接查阅原始课件。",
...
def main():
    parser = argparse.ArgumentParser(description="课件 RAG 问答")
    parser.add_argument("--query",         type=str,  default=None)
    parser.add_argument("--day",           type=str,  default=None, help="按天过滤，如 DAY1")
    ...
    filter_meta = {"day": args.day} if args.day else None
