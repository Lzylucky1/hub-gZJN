...
app = FastAPI(
    title       = "课件 RAG 问答服务",
    description = "FAISS + BM25 混合检索 + DashScope，含教学调试接口",
    version     = "1.0.0",
    lifespan    = lifespan,
)
...
class QueryRequest(BaseModel):
    question:   str           = Field(...,  example="什么是ADAM优化器")
    day:        Optional[str] = Field(None, example="DAY1")
...
def _filter_meta(req: QueryRequest) -> Optional[dict]:
    return {"day": req.day} if req.day else None
