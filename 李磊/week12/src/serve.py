"""
FastAPI HTTP 服务，提供流式 SSE 接口给 Web UI（Function Calling 版）

接口：
  POST /query/fc      - Function Calling 版 ReAct，流式返回每步
  GET  /health        - 健康检查
...
async def _stream_react(question: str, max_steps: int):
    """
    ...
    yield _sse({"type": "start", "question": question, "mode": "fc"})
...
async def query_fc(req: QueryRequest):
    return StreamingResponse(
        _stream_react(req.question, req.max_steps),
