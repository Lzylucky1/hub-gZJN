...
SYSTEM_PROMPT = (
    ...
    "涉及天气时调用 get_weather。本回合你可以一次调用多个工具，也可以在多轮中继续调用工具。"
)


def run(client, model: str, question: str, verbose: bool = True) -> dict:
    """
    循环闭环：提问 → 模型输出 tool_call → 执行 → 回填 → 继续请求，直到不再调用工具。
    ...
    for round_idx in range(1, max_rounds + 1):
        ...
        messages.append(msg)
        if verbose and round_idx > 1:
            print(f"  → [loop] 第 {round_idx} 轮继续调用工具")
