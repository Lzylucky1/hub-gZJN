...
from src.weather_backend import get_city_coordinates, get_weather_by_coordinates  # noqa: E402
...
TOOLS_SCHEMA = [
    ...
    },
    {
        ...
        "description": "查询指定城市的经纬度坐标。城市用中文名，如 '宁德'、'北京'。",
        "parameters": {
            "type": "object",
            "properties": {
    ...
    },
    {
        ...
        "description": "根据经纬度查询当前天气及未来3天预报。",
        "parameters": {
            "type": "object",
            "properties": {
...
SYSTEM_PROMPT = (
    ...
    "涉及天气时先调用 get_city_coordinates 查询城市经纬度，再调用 get_weather_by_coordinates 查询天气。本回合你可以一次调用多个工具。"
)
...
def run(client, model: str, question: str, verbose: bool = True) -> dict:
    ...
    # 【教学时刻 3】：循环执行模型输出的工具调用，支持链式调用。
    max_tool_rounds = 5
    round_index = 0
    while getattr(msg, "tool_calls", None):
        if round_index >= max_tool_rounds:
            tool_call_log.append({"error": f"超过最大工具调用轮次 {max_tool_rounds}"})
            break
        round_index += 1
        messages.append(msg)
...
def main():
    ...
    parser.add_argument("--question", "-q", default="厦门的天气如何？", help="单个问题")
