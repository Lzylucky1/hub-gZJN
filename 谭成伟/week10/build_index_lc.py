...
def load_documents():
    ...
    if not pdf_files:
        raise FileNotFoundError(f"{RAW_DIR} 目录下没有 PDF 文件，请先将PDF文件存放至 {RAW_DIR} 中")
...
def build_vectorstore(chunks, embeddings):
    ...
    arr = faiss.serialize_index(vectorstore.index)
    ...
    logger.info(f"  index.faiss: {index_path.stat().st_size // 1024} KB")
    return vectorstore
