...
# 会话记忆存储：session_id -> 历史消息列表，用于多轮对话上下文延续
_SESSIONS: dict = {}
...
def run(question: str, max_steps: int = 10, session_id: str = None) -> Generator[dict, None, None]:
    ...
    # 会话记忆：若 session_id 命中历史，则复用之前的对话上下文（含系统提示与历史轮次）
    ...
    # 达到最大步数时也保存已积累的对话上下文，避免中途工具调用历史丢失
    if session_id:
        _SESSIONS[session_id] = messages
