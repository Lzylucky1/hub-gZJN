...
def get_city_info(city: str) -> str:
    ...
    Returns:
        JSON 格式的城市信息，包含：latitude(纬度), longitude(经度), city_name(城市名), admin1(省份), country(国家)
        查询失败时返回错误描述字符串
    """
    with httpx.Client(timeout=10.0) as client:
        ...
        return json.dumps({
            ...
            "admin1": admin1,
            "country": country,
            "location": f"{country} {admin1} {city_name}".strip(),
        }, ensure_ascii=False)


def get_weather_by_coords(lat: float, lon: float, city_name: str = "",
                          admin1: str = "", country: str = "") -> str:
    """
    根据经纬度查询当前天气及未来3天预报。
    ...
    with httpx.Client(timeout=10.0) as client:
        ...
        if not location_str:
            location_str = f"{lat:.2f}°N, {lon:.2f}°E"
...
def get_weather(city: str) -> str:
    """
    查询指定城市的当前天气及未来3天预报。
    （内部依次调用 get_city_info + get_weather_by_coords，保持向后兼容）
