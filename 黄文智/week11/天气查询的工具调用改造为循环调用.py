...
from src.weather_backend import get_location  # noqa: E402
from src.weather_backend import get_weather_by_coords  # noqa: E402
...
TOOLS_SCHEMA = [
    ...
    },
    {
        ...
        "function":{
            "name": "get_location",
            "description": "查询指定城市的经纬度坐标（多轮天气查询的第一步）。返回JSON格式，包含latitude和longitude字段",
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function":{
            ...
            "description":"根据经纬度查询天气（多轮天气查询的第二步）。需要先调用get_location获取坐标。",
            "parameters": {
                "type": "object",
                "properties": {
...
TOOL_DISPATCH = {
    ...
    "get_location": get_location,           # 新增
    "get_weather_by_coords": get_weather_by_coords,  # 新增
}
...
SYSTEM_PROMPT = (
    ...
    "涉及天气查询时，必须先调用 get_location 获取城市坐标，然后用返回的经纬度调用 get_weather_by_coords 查询天气。"
    "获取到最终结果后，请直接总结回答，不要再调用工具。"
)
...
def dispatch_tool_call(name: str, args: dict) -> str:
    ...
    if name == "get_location":
        return get_location(args.get("city", ""))
    elif name == "get_weather_by_coords":
        return get_weather_by_coords(args.get("latitude", 0.0), args.get("longitude", 0.0))
    elif name == "search_annual_report":
        return search_annual_report(
...
def run_loop(client, model, question: str, verbose: bool = True) -> dict:
    """
    执行单轮对话：问题 → 工具调用 → 结果总结
    """
    messages = [{"role": "user", "content": question}]
    tool_calls_made = []
    max_calls = 5  # 最多调用5次工具



     # 多轮工具调用循环
    while len(tool_calls_made) < max_calls:  # 添加次数限制
        if verbose:
            print(f"[llm] 思考中...")
