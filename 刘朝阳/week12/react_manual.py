...
def run(question: str, history: list = None, max_steps: int = 10, verbose: bool = True) -> Generator[dict, None, None]:
    """
    ...
    history: 可选的多轮对话历史，格式 [{"role": "user"/"assistant", "content": str}, ...]
             手写版将其压成文本块拼入当前 user 消息，避免干扰 Thought/Action 格式解析
    """
...
def run_and_print(question: str, max_steps: int = 10, history: list = None) -> str:
    print(f"\n{'='*60}")
    print(f"问题: {question}")
    ...
    for step_data in run(question, history=history, max_steps=max_steps):
        step_count += 1
