...
# 工具调用循环上限：模型反复调工具时最多允许这么多轮，超过则强制收尾（防死循环）
MAX_TOOL_ROUNDS = 5
...
SYSTEM_PROMPT_NAMED = (
    ...
    "不在库内的公司请明确告知，不要臆测。本回合可一次调用多个不同工具；"
    "但当用户要查多个城市的天气时，请逐个城市 run_cli(command='weather', args={'city': ...})，"
    "每次只查一个城市，看到上一个结果后再查下一个——以此触发多轮工具调用循环。"
)
...
SYSTEM_PROMPT_BASH = (
    ...
    "不在库内的明确告知。本回合可一次调用多个不同工具；"
    "但当用户要查多个城市的天气时，请逐个城市执行 fincli weather --city <城市>，"
    "每次只查一个城市，看到上一个结果后再查下一个——以此触发多轮工具调用循环。"
)
...
def run(client, model: str, question: str, mode: str, verbose: bool = True) -> dict:
    ...
    # 【教学时刻改】：单轮 → 循环。只要模型还输出 tool_calls，就继续执行→回填→再请求，
    # 直到模型给出不再调用工具的最终回答；达到 MAX_TOOL_ROUNDS 上限则强制收尾（防死循环）。
    while msg.tool_calls:
        rounds += 1
        if rounds > MAX_TOOL_ROUNDS:
            if verbose:
                print(f"  → [loop] 达到最大轮次 {MAX_TOOL_ROUNDS}，强制收尾")
            resp = client.chat.completions.create(
        ...
        # 下一轮请求：模型看到工具结果，可能继续调工具，也可能直接给最终回答
        resp = client.chat.completions.create(
            model=model, messages=messages, tools=tools_schema, tool_choice="auto",
        )
        msg = resp.choices[0].message
    ...
    if verbose:
        print(f"  → [llm] 最终回答（{elapsed:.1f}s，共 {rounds} 轮工具调用）")
...
DEMO_QUESTIONS = [
    "宁德时代2023年营收和净利润是多少？",
    "查询武汉、北京、上海三个城市的天气。",  # 循环调用：逐城市调 weather，触发多轮循环
