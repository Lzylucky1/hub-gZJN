...
SYSTEM_PROMPT = (
    ...
    "涉及天气时调用 get_weather。本回合可一次调用多个不同工具（如同时查年报与单城市天气）；"
    "但当用户要查多个城市的天气时，请逐个城市调用 get_weather，每次只查一个城市，"
    "看到上一个结果后再查下一个——以此触发多轮工具调用循环。"
)
...
def run(client, model: str, question: str, verbose: bool = True) -> dict:
    ...
    # 【教学时刻 3 改】：单轮 → 循环。只要模型还输出 tool_calls，就继续
    # 执行→回填→再请求，直到模型给出不再调用工具的最终回答；
    # 达到 MAX_TOOL_ROUNDS 上限则强制收尾，防止模型反复调工具死循环。
...
DEMO_QUESTIONS = [
    "宁德时代2023年营收和净利润是多少？",
    "查询武汉、北京、上海三个城市的天气。",  # 循环调用：逐城市调 get_weather，触发多轮循环
